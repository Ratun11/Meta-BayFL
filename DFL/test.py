...
(x_train_full, y_train_full), (x_test, y_test) = fashion_mnist.load_data()  # Change this to mnist for using MNIST data
# Selecting 10% of the data
indices = np.arange(len(x_train_full))
np.random.shuffle(indices)
reduced_size = int(0.01 * len(indices))
...
def federated_learning_simulation(client_data, x_test, y_test, global_epochs=100):
    ...
    for global_epoch in range(global_epochs):
        ...
        print(f"Global Epoch {global_epoch+1}/{global_epochs}: Test Accuracy: {accuracy}")
    return global_model

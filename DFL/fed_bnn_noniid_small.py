...
def load_data():
    ...
    # Normalize and invert the images
    x_train_full = 1 - x_train_full / 255.0
    x_test = 1 - x_test / 255.0

    # Convert labels to one-hot encoding
    y_train_full_oh = tf.keras.utils.to_categorical(y_train_full)
    y_test_oh = tf.keras.utils.to_categorical(y_test)

    # Selecting 10% of the data
    indices = np.arange(len(x_train_full))
    np.random.shuffle(indices)
    reduced_size = int(1 * len(indices))

    x_train = x_train_full[indices[:reduced_size]]
    y_train_oh = y_train_full_oh[indices[:reduced_size]]

    # Splitting 10% data for federated learning
    num_clients = 3
...
federated_model = federated_learning_simulation(client_data, create_model, x_test, y_test_oh, rounds=100)
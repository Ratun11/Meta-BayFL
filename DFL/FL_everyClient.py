...
# Define federated training parameters
num_clients = 3
batch_size = 64
...
def train_client_model(client_id, client_model, client_train_loader):
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f"Client: {client_id}, Epoch [{epoch + 1}/{num_epochs}], Time: {epoch_time:.2f}, Loss: {epoch_loss:.4f}, Accuracy: {epoch_accuracy:.2f}%")
...
for client_id, client_dataset in enumerate(client_datasets):
    ...
    client_model = train_client_model(client_id, client_model, client_train_loader)
    client_models.append(client_model)

# Aggregate the client models (e.g., averaging their weights) to obtain the global model
aggregated_model = CNN()
for client_model in client_models:
    for aggregated_param, client_param in zip(aggregated_model.parameters(), client_model.parameters()):
        aggregated_param.data += client_param.data

# Now you can use the aggregated_model for inference or further training.

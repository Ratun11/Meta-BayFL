...
def federated_learning_simulation(client_data, model_fn, rounds=1, learning_rates_to_test=None):
    """
    Simulate federated learning with client-specific learning rates.
    ...
    :param best_learning_rates: List of best learning rates for each client
    :return: Trained model
    """
    global_model = model_fn()  # Initialize with a default learning rate
    ...
    for round_num in range(rounds):
        ...
        for client_index, (x, y) in enumerate(client_data):
            # Testing learning rates for each client and storing the best ones
            best_lrs_for_client = test_learning_rates(create_model, client_data[client_index], learning_rates_to_test)

            # Use the best learning rate for each client
            client_lr = best_lrs_for_client
            local_model = model_fn(client_lr)
...
# Modified function to create a new model instance with a variable learning rate
...
# Splitting data into simulated clients with varying data sizes
num_clients = 3
# Define learning rates to test
learning_rates_to_test = [0.001, 0.05, 0.01]
client_data = []

# Evenly splitting x_train and y_train into num_clients parts
client_train_dataset = np.array_split(x_train, num_clients)
client_train_labels = np.array_split(y_train, num_clients)

# Creating client_data by combining x and y for each client
client_data = [(client_train_dataset[i], client_train_labels[i]) for i in range(num_clients)]


federated_model = federated_learning_simulation(client_data, create_model, rounds=10, learning_rates_to_test=learning_rates_to_test)
